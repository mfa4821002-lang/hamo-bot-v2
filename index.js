...
async function verifyUser(ctx, next) {
    ...
    if (!isMember && !isDeveloper) {
        return ctx.replyWithPhoto(global.pp, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: "▶️ YouTube channel  ", url: YOUTUBE_LINK }],
                    [{ text: "📷 WhatsApp channel  ", url: INSTAGRAM_LINK }],
                    [{ text: "🔵 Telegram channel  ", url: CHANNEL_INVITE_LINK }],
                    [{ text: "🔹 Telegram group  ", url: GROUP_LINK }],
                    [{ text: "🔄 Check after subscription  ", callback_data: "check_membership" }]
                ]
            }
        });
    } else {
    ...
    try {
        ...
        if (XeonBotInc.callbackQuery.data === "check_membership") {
            ...
            await XeonBotInc.answerCbQuery( 
                isMember ? "✅ Thank you for subscribing to our channels to follow the latest news. You can now use the bot." : "❌ Sorry, dear, you need to subscribe to the channels first to follow the latest bot news.",
                { show_alert: true }
        ...
        const username = user.username ? user.username : "Midoxkill";
    ...
    try {
        ...
        if (profilePhotos.photos.length > 0) {
            ...
            await XeonBotInc.replyWithPhoto(ownerPhoto, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        [{ text: 'OWNER', url: "https://t.me/hamoaljin" }, { text: 'CHANNEL', url: "https://t.me/hamoaljin" }, { text: 'GROUP', url: "https://t.me/+V6MuqLNOtTs4NzE0" }]
                    ]
                }
            });
        } else {
        ...
        await XeonBotInc.reply(lang.first_chat(BOT_NAME, user.full_name), {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: 'OWNER', url: "https://t.me/hamoaljin" }, { text: 'GROUP', url: "https://t.me/+V6MuqLNOtTs4NzE0" }]
                ]
            }
        });
    }
...
*Hold on it to copy the ID.*`;
    ...
    // Execute XeonTele8.js only if the user is verified
    require("./XeonTele9")(XeonBotInc, bot);
    ...
    if (!Array.isArray(adminIDs) || !adminIDs.includes(userID)) {
        return ctx.replyWithMarkdownV2(
            ...
            `📩 *Send proof to the owner @DGXeon*`, 
            {
                reply_markup: {
                    inline_keyboard: [
                        [{ text: "📢 Telegram Channel", url: "https://t.me/hamoaljin" }],
