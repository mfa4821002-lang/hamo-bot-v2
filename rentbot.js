...
const PHONENUMBER_MCC = require('./xxxx.json');
...
/* ===== QUOTED MESSAGE SAFE HANDLING ===== */

let context = m.msg?.contextInfo || null
let quoted = m.quoted = context?.quotedMessage || null
...
m.quoted.chat = context.remoteJid || m.chat
m.quoted.isBaileys =
m.quoted.id?.startsWith('BAE5') && m.quoted.id.length === 16

m.quoted.sender = XeonBotInc.decodeJid(context.participant)
m.quoted.fromMe =
m.quoted.sender === XeonBotInc.decodeJid(XeonBotInc.user.id)
...
m.quoted.mentionedJid = context.mentionedJid || []
...
m.quoted.delete = () =>
XeonBotInc.sendMessage(m.quoted.chat, { delete: vM.key })

m.quoted.copyNForward = (jid, forceForward = false, options = {}) =>
XeonBotInc.copyNForward(jid, vM, forceForward, options)

m.quoted.download = () =>
XeonBotInc.downloadMediaMessage(m.quoted)
}

/* ===== NORMAL MESSAGE ===== */

if (m.msg?.url)
m.download = () => XeonBotInc.downloadMediaMessage(m.msg)
